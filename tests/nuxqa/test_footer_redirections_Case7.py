"""
test_footer_redirections_Case7.py - Test Case 7: Verificar redirecciones del Footer

Este test verifica que los links del footer redirigen correctamente a sitios externos/internos.
Se ejecuta para los 4 footer links especificados y en ambos ambientes (QA4 y QA5).
"""
...
def test_footer_redirections(driver, base_url, db, footer_link, browser, screenshots_mode, language, request):
    ...
    - footer_link: Controlado por --footer-link (default: all)
    ...
    allure.dynamic.tag(footer_link)       # Tag por link: vuelos, trabajos, aviancadirect, articulos
    allure.dynamic.tag(env)               # Tag por ambiente: NUXQA4, NUXQA5
    allure.dynamic.tag("Case7")           # Tag general del caso

    # Descripción detallada
    allure.dynamic.description(
        f"Verifica que al hacer click en el footer link '{link_name}' en el sitio {env}, "
    ...
    logger.info(f"========== Starting test: Footer redirection to '{link_name}' on {base_url} ==========")
    ...
    with allure.step(f"Click on footer link: {link_name}"):
        ...
        # Capturar screenshot solo si modo es "all"
        if screenshots_mode == "all":
            allure.attach(driver.get_screenshot_as_png(), name=f"02_After_Click_{footer_link}", attachment_type=allure.attachment_type.PNG)
    ...
    with allure.step(f"Verify redirection to {link_name}"):
        ...
        allure.dynamic.title(f"Footer Link: {link_name} | Browser: {browser.capitalize()} | Env: {env} | Lang: {selected_language} ({language_mode})")
        ...
        assert success, f"Footer redirection to '{link_name}' failed: {message}"
    ...
    test_name = f"Case7_{footer_link}_{env}_{browser}_{selected_language}"
    ...
    db.save_test_result(
        ...
        footer_link=footer_link,  # Campo específico del Case 7
